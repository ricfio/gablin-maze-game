.PHONY: help backup build clean install shell start
.DEFAULT_GOAL := help

CURRDIR := $(shell pwd)
WORKDIR := /workspaces/$(notdir $(CURRDIR))
DOCKER_IMAGE := $(notdir $(CURRDIR))
PORT := 40080
...
shell: ## Login into docker developer container
	...
	docker run -it --rm -u node -p $(PORT):$(PORT) -v $(CURRDIR):$(WORKDIR) -w $(WORKDIR) $(DOCKER_IMAGE) bash
